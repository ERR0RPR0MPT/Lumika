...
func AddInput() {
	LogPrint("", AddStr, "使用 \""+os.Args[0]+" help\" 查看帮助")

	fileDir := LumikaWorkDirPath
	fileEncodeDir := LumikaEncodePath

	if _, err := os.Stat(fileDir); os.IsNotExist(err) {
		LogPrint("", AddStr, ErStr, "输入文件夹不存在:", err)
		return
	}

	LogPrint("", AddStr, "当前编码目录:", fileDir)

	fileDict, err := GenerateFileDxDictionary(fileEncodeDir, ".fec")
	if err != nil {
		LogPrint("", AddStr, ErStr, "无法生成文件列表:", err)
		return
	}

	if len(fileDict) != 0 {
		LogPrint("", AddStr, ErStr, "错误：检测到目录下存在 .fec 文件，请先删除 .fec 文件再进行添加")
		return
	}
	...
	for {
		if len(fileDict) == 0 {
			LogPrint("", AddStr, ErStr, "当前目录下没有文件，请将需要编码的文件放到当前目录下")
			return
		}
		LogPrint("", AddStr, "请选择需要编码的文件，输入索引并回车来选择")
		LogPrint("", AddStr, "如果需要编码当前目录下的所有文件，请直接输入回车")
		for index := 0; index < len(fileDict); index++ {
			LogPrint("", "Encode:", strconv.Itoa(index)+":", fileDict[index])
		}
		result := GetUserInput("")
		if result == "" {
			LogPrint("", AddStr, "注意：开始编码当前目录下的所有文件")
			...
			if err != nil {
				LogPrint("", AddStr, ErStr, "输入索引不是数字，请重新输入")
				continue
			}
			if index < 0 || index >= len(fileDict) {
				LogPrint("", AddStr, ErStr, "输入索引超出范围，请重新输入")
				continue
			}
	...
	// 设置M的值
	LogPrint("", AddStr, "请输入 M 的值(0<=M<=256)，M 为最终生成的总切片数量。默认：\""+strconv.Itoa(AddMLevel)+"\"")
	defaultM, err := strconv.Atoi(GetUserInput(""))
	if err != nil {
		LogPrint("", AddStr, "自动设置 M = "+strconv.Itoa(AddMLevel))
		defaultM = AddMLevel
	}
	if defaultM == 0 {
	...
	// 设置K的值
	LogPrint("", AddStr, "请输入 K 的值(0<=K<=256)，K 为恢复原始文件所需的最少完整切片数量。默认：\""+strconv.Itoa(AddKLevel)+"\"")
	defaultK, err := strconv.Atoi(GetUserInput(""))
	if err != nil {
		LogPrint("", AddStr, "自动设置 K = "+strconv.Itoa(AddKLevel))
		defaultK = AddKLevel
	}
	if defaultK == 0 {
	...
	if defaultK > defaultM {
		LogPrint("", AddStr, ErStr, "错误: K 的值不能大于 M 的值，自动设置 K = M = "+strconv.Itoa(defaultM))
		defaultK = defaultM
	}

	// 设置MG的值
	LogPrint("", AddStr, "请输入 MG 的值(0<=MG<=256)，MG 为帧数据的总切片数量。默认：\""+strconv.Itoa(AddMGLevel)+"\"")
	MGValue, err := strconv.Atoi(GetUserInput(""))
	if err != nil {
		LogPrint("", AddStr, "自动设置 G = "+strconv.Itoa(AddMGLevel))
		MGValue = AddMGLevel
	}
	if MGValue == 0 {
		LogPrint("", AddStr, ErStr, "错误: G 的值不能为 0，自动设置 G = "+strconv.Itoa(AddMGLevel))
		MGValue = AddMGLevel
	}

	// 设置KG的值
	LogPrint("", AddStr, "请输入 KG 的值(0<=KG<=256)，KG 为恢复帧数据所需的最少完整切片数量。默认：\""+strconv.Itoa(AddKGLevel)+"\"")
	KGValue, err := strconv.Atoi(GetUserInput(""))
	if err != nil {
		LogPrint("", AddStr, "自动设置 G = "+strconv.Itoa(AddKGLevel))
		KGValue = AddKGLevel
	}
	if KGValue == 0 {
		LogPrint("", AddStr, ErStr, "错误: G 的值不能为 0，自动设置 G = "+strconv.Itoa(AddKGLevel))
		KGValue = AddKGLevel
	}

	if KGValue > MGValue {
		LogPrint("", AddStr, ErStr, "错误: KG 的值不能大于 MG 的值，自动设置 KG = MG = "+strconv.Itoa(MGValue))
		KGValue = MGValue
	}

	// 设置默认的分辨率大小
	LogPrint("", AddStr, "请输入分辨率大小，例如输入32则分辨率为32x32。默认：\""+strconv.Itoa(EncodeVideoSizeLevel)+"\"")
	videoSize, err := strconv.Atoi(GetUserInput(""))
	if err != nil {
		LogPrint("", AddStr, "自动设置分辨率大小为", EncodeVideoSizeLevel)
	...
	// 设置默认的帧率大小
	LogPrint("", AddStr, "请输入帧率大小。默认：\""+strconv.Itoa(EncodeOutputFPSLevel)+"\"")
	outputFPS, err := strconv.Atoi(GetUserInput(""))
	if err != nil {
		LogPrint("", AddStr, "自动设置帧率大小为", EncodeOutputFPSLevel)
		outputFPS = EncodeOutputFPSLevel
	}
	if outputFPS <= 0 {
		LogPrint("", AddStr, ErStr, "错误: 帧率大小不能小于等于 0，自动设置帧率大小为", EncodeOutputFPSLevel)
		outputFPS = EncodeOutputFPSLevel
	}

	// 设置默认最大生成的视频长度限制
	LogPrint("", AddStr, "请输入最大生成的视频长度限制(单位:秒)，默认：\""+strconv.Itoa(EncodeMaxSecondsLevel)+"\"")
	encodeMaxSeconds, err := strconv.Atoi(GetUserInput(""))
	if err != nil {
		LogPrint("", AddStr, "自动设置最大生成的视频长度限制为", EncodeMaxSecondsLevel, "秒")
		encodeMaxSeconds = EncodeMaxSecondsLevel
	}
	if encodeMaxSeconds <= 0 {
		LogPrint("", AddStr, ErStr, "错误: 最大生成的视频长度限制不能小于等于 0，自动设置最大生成的视频长度限制为", EncodeMaxSecondsLevel)
		encodeMaxSeconds = EncodeMaxSecondsLevel
	}

	// 设置默认使用的 FFmpeg 预设模式
	LogPrint("", AddStr, "请输入使用的 FFmpeg 预设模式，例如：\"ultrafast\"。默认：\""+EncodeFFmpegModeLevel+"\"")
	encodeFFmpegMode := GetUserInput("")
	if encodeFFmpegMode == "" {
		LogPrint("", AddStr, "自动设置使用的 FFmpeg 预设模式为", EncodeFFmpegModeLevel)
		encodeFFmpegMode = EncodeFFmpegModeLevel
	}

	// 设置处理使用的线程数量
	LogPrint("", AddStr, "请输入处理使用的线程数量。默认(CPU核心数量)：\""+strconv.Itoa(runtime.NumCPU())+"\"")
	encodeThread, err := strconv.Atoi(GetUserInput(""))
	if err != nil {
		LogPrint("", AddStr, "自动设置处理使用的线程数量为", runtime.NumCPU())
...
func AddExec(fileNameList []string, defaultM int, defaultK int, MGValue int, KGValue int, videoSize int, outputFPS int, encodeMaxSeconds int, encodeThread int, encodeFFmpegMode string, defaultSummary string, UUID string) {
	...
	for ai, fileName := range fileNameList {
		LogPrint(UUID, AddStr, "开始编码第"+strconv.Itoa(ai+1)+"个文件:", fileName)
		if fileName == "" {
			LogPrint(UUID, AddStr, ErStr, "错误: 文件名不能为空，跳过该文件")
			continue
		}
		// 设置默认文件名
		...
		// 开始生成 .fec 文件
		LogPrint(UUID, AddStr, "开始生成 .fec 文件")
		...
		for i, shard := range shards {
			...
			LogPrint(UUID, AddStr, "写入 .fec 文件:", outfn)
		...
		LogPrint(UUID, AddStr, "Base64 配置生成完成，开始写入文件:", fecFileConfigFilePath)
