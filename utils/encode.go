...
func Encode(fileDir string, videoSize int, outputFPS int, maxSeconds int, MGValue int, KGValue int, encodeThread int, encodeFFmpegMode string, auto bool,
	...
	for fileIndexNum, filePath := range filePathList {
		...
		go func(fileIndexNum int, filePath string) {
			...
			common.LogPrintln(UUID, common.EnStr, filePath, "编码完成")
			common.LogPrintln(UUID, common.EnStr, "使用配置：")
			//common.LogPrintln(UUID, common.EnStr, "  ---------------------------")
			//common.LogPrintln(UUID, common.EnStr, "  输入文件:", filePath)
			//common.LogPrintln(UUID, common.EnStr, "  输出文件:", outputFilePath)
...
func EncodeForAndroid(fileDir string, videoSize int, outputFPS int, maxSeconds int, MGValue int, KGValue int, encodeThread int, encodeFFmpegMode string, auto bool,
	...
	for fileIndexNum, filePath := range filePathList {
		...
		go func(fileIndexNum int, filePath string) {
			...
			outputTempDir := filepath.Join(filepath.Dir(outputFilePath), outputTempDirName)
			...
			for {
				...
				if len(fileData) == 0 {
					if shardsInsideNum != 0 {
						...
						// 生成带空白数据的图像
						err = png.Encode(file, imgBlankStart)
						if err != nil {
							errorChan <- &common.CommonError{Msg: "无法生成带空白数据的图像:" + err.Error()}
							return
						}
						...
						// 生成带空白数据的图像
						err = png.Encode(file, imgBlankEnd)
						if err != nil {
							errorChan <- &common.CommonError{Msg: "无法生成带空白数据的图像:" + err.Error()}
							return
						}
			...
			common.LogPrintln(UUID, common.EnStr, "开始调用 FFmpeg Android 库生成视频")
			FFmpegCmd := []string{
				"-y",
				"-vcodec", "png",
				"-r", fmt.Sprintf("%d", outputFPS),
			...
			if err != nil {
				common.LogPrintln(UUID, common.EnStr, common.ErStr, "无法删除临时目录:", err)
			} else {
				common.LogPrintln(UUID, common.EnStr, "已删除临时目录")
			}
...
func EncodeWithoutPipe(fileDir string, videoSize int, outputFPS int, maxSeconds int, MGValue int, KGValue int, encodeThread int, encodeFFmpegMode string, auto bool,
	...
	for fileIndexNum, filePath := range filePathList {
		...
		go func(fileIndexNum int, filePath string) {
			...
			common.LogPrintln(UUID, common.EnStr, "开始调用 FFmpeg 生成视频")
			...
			common.LogPrintln(UUID, common.EnStr, "FFmpeg 子进程执行完成")
